...
Interpolator::Interpolator() {
    ...
    base_order = 10;     // numero pi� vicino a cui normalizzare i parametri
...
void Interpolator::setOrder(int par_num, double order, std::string& f_interp) {      // ad esempio setOrder(0, 0.001) pone al posto del parametro "a" (numero 0) la stringa "a*0.001"
    ...
    // La sostituzione sar� parametro*order, ad es. "b*1000"
...
//cerco il valore di x[i] in funzione di y[i] con bisezione anzich� calcolare la funzione inversa che mi � impossibile in generale
double Interpolator::xFunction(std::vector<double> par, int i) {
    ...
    auto f = [this, i](double x) -> double {
        ...
        return y[i] - expression.value(); // ora 'i' � catturato
    };
    ...
    for (int k = 0; k < max_iter; k++) {
        ...
        // Controllo se il risultato � sufficientemente vicino a 0 o se l'intervallo � piccolo
    ...
    // Se il numero massimo di iterazioni � stato raggiunto
